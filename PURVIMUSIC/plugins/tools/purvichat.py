...
MONGO_URL = os.environ.get("MONGO_URL", "mongodb+srv://PURVIMUSIC:PurviBots@cluster0.eclc5.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
    ...
    if message.from_user:
        ...
        if user not in (
           ...
           return await message.reply_text(
                "**ᴀʀᴇ ʏᴏᴜ sᴜʀᴇ ʏᴏᴜ ᴀʀᴇ ᴀɴ ᴀᴅᴍɪɴ ?🧐**"
            )
    is_v = v.find_one({"chat_id": message.chat.id})
    if not is_v:
        v.insert_one({"chat_id": message.chat.id})
        await message.reply_text(f"**ᴄʜᴀᴛʙᴏᴛ ᴅɪsᴀʙʟᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ 💔!**")
    if is_v:
...
async def chatboton(client, message):
    ...
    if is_v:
        ...
        await message.reply_text(f"**ᴄʜᴀᴛʙᴏᴛ ɪs ᴇɴᴀʙʟᴇᴅ 🥳**")
